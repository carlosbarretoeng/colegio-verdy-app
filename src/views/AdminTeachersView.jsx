...
const PROFESSORES = [
  { id: 'p1', nome: 'Mariana Lopes', disciplina: 'Linguagem', status: 'ativo' },
  { id: 'p2', nome: 'Rafael Costa', disciplina: 'Matemática', status: 'ativo' },
  { id: 'p3', nome: 'Camila Nunes', disciplina: 'Artes', status: 'inativo' },
  { id: 'p4', nome: 'João Pedro Lima', disciplina: 'Educação Física', status: 'ativo' }
];

const ITEMS_PER_PAGE = 5;

export default function AdminTeachersView() {
  const [professores] = useState(PROFESSORES);
  const [filtroNome, setFiltroNome] = useState('');
  const [filtroStatus, setFiltroStatus] = useState('todos');
  const [pagina, setPagina] = useState(1);

  const filtrados = useMemo(() => professores.filter((professor) => {
  ...
  }), [professores, filtroNome, filtroStatus]);
  ...
  return (
    <div className="flex flex-col gap-4 max-w-5xl mx-auto">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">Controle do quadro docente da escola.</p>
      </div>
      ...
      <div className="glass-panel p-5 border border-slate-200/60">
        ...
        {paginados.map((professor) => (
          <div key={professor.id} className="py-2 border-b border-slate-100 last:border-b-0 flex items-center justify-between">
            <div>
              <div className="font-medium text-slate-800">{professor.nome}</div>
              <div className="text-sm text-slate-500">{professor.disciplina}</div>
            </div>
            <span className={`text-xs px-2 py-1 rounded-full font-semibold ${professor.status === 'ativo' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600'}`}>{professor.status}</span>
          </div>
        ))}
